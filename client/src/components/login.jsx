...
setloginerror(msg);
  ...
  <div className="text-red-600 text-md mt-2 text-center">
    {loginerror}
      ...
      <div className="flex p-2 mt-4 items-center justify-center px-2 py-4">
        <button onClick={handleLogin} className='bg-blue-600 transition-transform active:scale-95 w-full rounded-md px-2 py-3 '>Login</button>
      </div>
  ...
  <div className="flex-grow h-px bg-zinc-300" />
        ...
        <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 3l18 18M4.5 4.5l15 15M9.53 9.53a3 3 0 014.95 3.18M3 12s3.75-6.75 9.75-6.75c2.188 0 4.188.75 6 2.1M21 12s-3.75 6.75-9.75 6.75a9.72 9.72 0 01-6-2.1" />
          </svg>
        ...
        <button onClick={()=>setPass(!showpass)} className='text-zinc-600 top-9 right-4 z-40 absolute'  >{ showpass ? <FiEyeOff size={20} /> : <FiEye size={20} />}</button>
        </div>
      <div className="flex p-2 mt-4 items-center justify-center px-2 py-4">
        <button onClick={handleSignup} className='bg-blue-600 transition-transform active:scale-95 w-full rounded-md px-2 py-3 ' >Create an account</button>
      </div>
